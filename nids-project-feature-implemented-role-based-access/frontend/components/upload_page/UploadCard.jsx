...
export default function UploadCard({ onResults }) {
  ...
  const runPipeline = async (fetchAction) => {
    ...
    try {
      // Stage 1: Parsing (Slightly faster)
      await new Promise(r => setTimeout(r, 800));
      setProgressStage(2);

      // Stage 2: Computation (The "heart" of the ML work)
      await new Promise(r => setTimeout(r, 1500));
      setProgressStage(3);

      // Stage 3: Detection (Integration)
      ...
      // We don't reset progressStage immediately so success can be felt, 
      // but setLoading(false) triggers parent navigation usually
      setLoading(false);
    }
  };
    ...
    <div className="w-full max-w-2xl mx-auto">
      <div className="rounded-3xl bg-white/5 backdrop-blur-xl border border-white/10 p-10 shadow-2xl relative overflow-hidden group">
        ...
        <div className="flex flex-col items-center text-center relative z-10">
          ...
          <div className="mt-8 flex items-center gap-6 opacity-30 grayscale saturate-0 pointer-events-none">
            <span className="text-[10px] font-black italic tracking-tighter">NIDS_SECURE</span>
            <span className="text-[10px] font-black italic tracking-tighter">P_NEURAL_ENG</span>
            <span className="text-[10px] font-black italic tracking-tighter">ZERO_TRUST_PRO</span>
          </div>
        </div>
      </div>
    </div>
