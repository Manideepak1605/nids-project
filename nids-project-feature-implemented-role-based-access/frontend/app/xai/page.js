...
export default function XAIPage() {
    ...
    React.useEffect(() => {
        const fetchXAIData = async () => {
            try {
                const response = await fetch('http://localhost:5000/api/xai');
                if (!response.ok) throw new Error('Failed to fetch XAI data');
        ...
        fetchXAIData();
        const interval = setInterval(fetchXAIData, 10000); // Poll every 10s
        return () => clearInterval(interval);
    }, [eventIdParam]);

    if (isLoading || !selectedEvent) {
        return (
            <div className="min-h-screen bg-black text-gray-200 p-8 flex items-center justify-center">
                <div className="text-violet-500 animate-pulse text-xl">Analyzing Local Traces...</div>
            </div>
        );
    }
