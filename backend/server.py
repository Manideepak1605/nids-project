...
def update_global_stats(new_results):
    ...
    for res in new_results:
        if res["Status"] == "BLOCK":
            ...
            stats["attack_types"][attack_type] = stats.get("attack_types", {}).get(attack_type, 0) + 1
        else:
            stats["allowed"] += 1
...
def nids_full_inference(flow_data, ae_assets, binary_assets, mc_assets):
    ...
    if not is_anomaly:
        return {"Status": "Allow", "Classification": "Benign", "MSE": float(mse), "Confidence": 1.0}
    ...
    if is_attack == 0:
        return {"Status": "Allow", "Classification": "Benign (Anomaly but Not Attack)", "MSE": float(mse), "Confidence": 0.85}
